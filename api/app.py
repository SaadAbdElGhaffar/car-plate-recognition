...
from config.settings import MODEL_PATH, CLASS_NAMES_PATH, TEMPLATE_FOLDER, STATIC_FOLDER
...
def index():
    if request.method == 'POST':
        ...
        if video_file:
            video_path = os.path.join(app.static_folder, 'input.mp4')
...
def stream_video():
    cap = cv2.VideoCapture(os.path.join(app.static_folder, 'input.mp4'))
    counter = []
